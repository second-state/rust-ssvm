...
fn main() {
    ...
    println!("{} a: {:?} b: {:?}", funcname, a_arr, b_arr);
   ...
   match funcname.as_str() {
        "add" => callData.extend(vec![119u8,22u8,2u8,247u8]),
        "sub" => callData.extend(vec![182u8,125u8,119u8,197u8]),
        "mul" => callData.extend(vec![200u8,164u8,172u8,156u8]),
        "div" => callData.extend(vec![163u8,145u8,193u8,91u8]),
        _ => {}
   }
   ...
   println!("{:?}", callData);
    ...
    match read_a_file(file_path) {
        Ok(code) => {
            ...
            println!("Output:  {:?}", output[0] as i32 + output[1] as i32 * (1 << 8) + output[2] as i32 * (1 << 16) + output[3] as i32 * (1 << 24));
